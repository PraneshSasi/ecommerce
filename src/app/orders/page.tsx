...
export default function OrdersPage() {
  ...
  }, [status, fetchOrders, fetchProfile, router]);
  ...
  // Use user's current profile address if available, otherwise fall back to order-time data
  const getShippingDetails = (order: Order) => {
    const hasProfileAddress = userProfile && userProfile.addressLine && userProfile.city && userProfile.state && userProfile.pincode;
    if (hasProfileAddress) {
      return {
        fullName: userProfile.name || order.fullName,
        phone: userProfile.phone || order.phone,
        addressLine: userProfile.addressLine || order.addressLine,
        city: userProfile.city || order.city,
        state: userProfile.state || order.state,
        pincode: userProfile.pincode || order.pincode,
      };
    }
    return {
              ...
              return (
                ...
                >
                  ...
                  <div className="grid gap-6 pt-6 lg:grid-cols-12 items-start">
                    ...
                    {/* Shipping address details — uses user's current profile data */}
                    <div className="lg:col-span-4 rounded-xl border border-white/5 bg-[#0c0c0f] p-4 space-y-4">
                      <div>
                        ...
                        <p className="text-xs font-black text-white uppercase font-mono">{shipping.fullName}</p>
                        <p className="mt-2 text-xs leading-5 text-gray-300 uppercase font-medium font-mono">
                          {shipping.addressLine}
                          <br />
                          {shipping.city}, {shipping.state} - {shipping.pincode}
                        </p>
                        <p className="mt-2 text-xs font-mono font-semibold text-gray-500">PHONE: {shipping.phone}</p>
                      </div>
